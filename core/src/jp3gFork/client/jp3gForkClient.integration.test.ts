...
function getAvailableTestImages(): string[] {
  ...
  try {
    ...
    console.error('Integration: Failed to read test images directory:', error);
...
describe.skip('Jp3gForkClient Integration Tests', () => {
  ...
  try {
    ...
    console.log(`✅ Primary test image loaded: ${primaryTestImage} (${testImageBuffer.length} bytes)`);
  } catch (error) {
    console.error('❌ Failed to load primary test image:', error);
    throw error;
  }
